...
VisionController::VisionController(zmq::context_t& ctx, QObject* parent)
    : QObject(parent), sub_(ctx, VisionCommon::AI_RESULTS_ENDPOINT, VisionCommon::TOPIC_DETECTIONS) {
...
void VisionController::run() {
    ...
    while (running_) {
        ...
        spdlog::info("[Front] detections: topic={} payload={}", topic, payload);

        emit detectionsReceived(QString::fromStdString(payload));
    }
