...
BluetoothController::BluetoothController(SubSocket& subSocket, QObject* parent)
    : QObject(parent),
...
void BluetoothController::start() {
    notifier_ = new QSocketNotifier(subSocket_.get_fd(), QSocketNotifier::Read, this);
    connect(notifier_, &QSocketNotifier::activated,
            this, &BluetoothController::onActivated);
...
void BluetoothController::onActivated() {
    while (true) {
        auto data = subSocket_.receivePayload();
        if (data) {
